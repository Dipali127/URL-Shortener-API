...
const createShortURL = async function (req, res) {
    try {
        ...
        const customShortcode = req.body.customShortcode;
        ...
        if (isExistURL) {
            return res.status(200).send({
                status: true, message: "longURL already exist",
                data: `${process.env.BASE_URL}/${isExistURL.shortCode}`
            });
        }
...
const redirectUrl = async function (req, res) {
    try {
        ...
        if (!isValidShortCode(shortCode)) {
            return res.status(400).send({
                status: false,
                message: " Invalid shortURL format. Please provide a valid shortURL "
            });
        }
...
const handleClick = async function (req, res) {
    try {
        ...
        if (isExistUrl) {
            let parseUrl = JSON.parse(isExistUrl);
            return res.status(200).send({
                status: true, message: "total clicks",
        ...
        return res.status(200).send({
            status: true, message: "total clicks", totalClicks:
